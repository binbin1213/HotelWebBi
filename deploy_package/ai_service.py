...
def simulate_insights(df):
    ...
    try:
        ...
        if 'revenue' in df.columns:
            ...
            insights["summary"] = f"总收入为 {total_revenue:.2f} 元。"
            ...
            if 'channel' in df.columns:
                ...
                if len(df) > 1:
                    ...
                    if float(bottom_channel['revenue']) / total_revenue * 100 < 15:
                        insights["recommendations"].append(
                            f"分析{bottom_channel['channel']}渠道的原因并制定改进措施，目标是将其收入提升至少30%，达到{float(bottom_channel['revenue'])*1.3:.2f}元。"
                        )
